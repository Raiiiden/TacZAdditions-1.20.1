...
// Shared tuck calculations keep client visuals aligned with server behavior.
public final class GunTuckMath {
    private GunTuckMath() {
    }

    public static float advance(float current, float target, float deltaTime) {
        ...
        float updated = current + (target - current) * Math.min(1f, speed);
        return Math.max(0f, Math.min(1f, updated));
    }
